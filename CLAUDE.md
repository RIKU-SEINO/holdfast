...
- **実装コード（テスト含む）はユーザー自身が書く。Claude は書かない。** これが最優先の約束。手を動かして Go を習得するのが目的なので、Claude が先回りして実装・テストを書いてはいけない。Claude の役割は (1) 設計の選択肢と推奨、(2) Go の癖・イディオムの指摘、(3) 何を書くべきかの方針・ヒント、までに留める。実際の Edit/Write はユーザーに委ねる。ユーザーが書いたコードのレビュー・採点・テスト実行・デバッグ支援は行ってよい。**ユーザーが明示的に「書いて」と言った場合のみ**コードを書く。
